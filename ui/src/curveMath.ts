...
export function normalizeCurve(curve: CurvePoint[]): CurvePoint[] {
  ...
  for (const [temp, duty] of curve) {
    ...
    points.set(Math.max(0, Math.min(150, Math.round(temp))), Math.max(0, Math.min(100, Math.round(duty))));
  }
...
export function pathForCurve(curve: CurvePoint[], width: number, height: number, maxTemp = 110): string {
  return curve
    .map(([temp, duty], index) => {
